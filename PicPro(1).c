...
if(*(imgaddr-40)==0)
{for(i=0;i<40;i++)
        {  if(*(imgaddr+i-40)==0&&*(imgaddr+i-40+1)==255&&RF==0)
           {RightB[0]=40+i;RF=1;}
          if(*(imgaddr-i-40+1)==0&&*(imgaddr-i-40)==255&&LF==0) //右边
          { 
            LeftB[0]=39-i;
            LF=1; 
          }
...
else if(*(imgaddr-40)==255)
{
for(i=0;i<40;i++)
        {  if(*(imgaddr+i-40)==0&&RF==0)
           {LeftB[0]=40+i;RF=1;RightB[0]=79;}
          if(*(imgaddr-i-40)==0&&LF==0) //右边
          { 
...
for(j=1;j<60;j++)
  {     
   ...
   {  
     if(*(imgaddr+1+midline[j-1]+i)==255&&RF==0&&midline[j-1]+i<80)
     { 
    ...
    if(*(imgaddr+1+midline[j-1]-i)==255&&LF==0&&(midline[j-1]-i>0||midline[j-1]-i==0))
     { 
...
uint8 you=0;
  ...
  if(shizi_flag==1){ for(int i=zuo-4;i<zuo+4;i++) {if(LeftB[i]==LeftB[zuo]) L++;  }} 
    if(shizi_flag==2){ for(int i=you-4;i<you+4;i++) {if(RightB[i]==RightB[you]) R++;  }} //修改
...
else if(Road_state!=RD_shizi&&block_flag>=1&&Dead==60)  //路障
{
